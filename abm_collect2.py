...
def initialize_cont_dict2(seed_list,pop_list,time_list,parameter_list):
    """Initialize the individual continuous properties vectors in a dictionary:
    """
    sub_parameter_dict = {}
    for seed_select in seed_list:
        for time_given in time_list:
            for pop_select in pop_list:
                type_out = get_population_name(pop_select)
                param_name1 = f"{type_out}_Cycle_S{seed_select}_T{str(time_given).replace('.', '_')}"
                param_name2 = f"{type_out}_Volume_S{seed_select}_T{str(time_given).replace('.', '_')}"
                sub_parameter_dict[param_name1] = []
                sub_parameter_dict[param_name2] = []

                for parameter_select in parameter_list:
                    # param_name = f"{parameter_select}_S{seed_select}_{type_out}_T{str(time_select).replace('.', '_')}"
                    param_name = f"{type_out}_{parameter_select}_S{seed_select}_T{str(time_given).replace('.', '_')}"
                    sub_parameter_dict[param_name] = []
...
def collect_cont_properties(sub_parameter_dict, parameter_list, pop_list, time_list, seed_list, N_in, H_in, C_in, D_in):
    '''
                ...
                for location in range(0, len(C_in)):
                    for position in range(0, 6):
                        if D_in['agents'][seed][time][height][location]['pop'][position] != -1:
                            ...
                            param_name2 = f"{type_out}_Volume_S{seed}_T{str(time_given).replace('.', '_')}"
                            sub_parameter_dict[param_name2].append(volume_info)

                            if D_in['agents'][seed][time][height][location]['cycle'][position] != -1:
                                cycle_info = D_in['agents'][seed][time][height][location]['cycle'][position]
                                param_name1 = f"{type_out}_Cycle_S{seed}_T{str(time_given).replace('.', '_')}"
                                sub_parameter_dict[param_name1].append(cycle_info)

    for seed_select in seed_list:
        json_file_name = os.path.join(
            args.param_path + 'VIVO_HET_GRAPH_' + C_HET + '_' + T_HET + '_' + POP_NAME + '_0' + str(
                seed_select) + '.PARAM.json')
        print('----------------------------')
    ...
    for seed_select in seed_list:
        for pop_select in pop_list:
            type_out = get_population_name(pop_select)
            for time_select in time_list:
                param_name1 = f"{type_out}_Cycle_S{seed_select}_T{str(time_select).replace('.', '_')}"
                param_name2 = f"{type_out}_Volume_S{seed_select}_T{str(time_select).replace('.', '_')}"
                cont_param1 = f"{type_out}_Cycle"
                cont_param2 = f"{type_out}_Volume"
                cont_param_dict[cont_param1].append(sub_parameter_dict[param_name1])
                cont_param_dict[cont_param2].append(sub_parameter_dict[param_name2])
                for parameter_select in parameter_list:
                    param_name = f"{type_out}_{parameter_select}_S{seed_select}_T{str(time_select).replace('.', '_')}"
                    cont_param = f"{type_out}_{parameter_select}"
                    cont_param_dict[cont_param].append(sub_parameter_dict[param_name])
...
for pkl_i in pkllist:
    ...
    sub_parameter_dict = initialize_cont_dict2(seed_list,pop_list,time_list,parameter_list)
    cont_param_dict = collect_cont_properties(sub_parameter_dict, parameter_list, pop_list, time_list, seed_list, N, H, C, D)
